...
export default function callFlutterwave(
  flutterWaveConfig: FlutterwaveConfig
): ({ callback, onClose }: InitializeFlutterwavePayment) => void {
  const state = useFWScript();

  function handleFlutterwavePayment({
    callback,
    onClose,
  }: InitializeFlutterwavePayment): void {
    if (state.$$__value?.error) throw new Error('Unable to load flutterwave payment modal');

    if (state.$$__value?.loaded === true) {
